ServerInfo = {
  ...
  health: function() {
    let res = '';
    if (typeof ServerInfo.settings.health == "function") {
      res = ServerInfo.settings.health();
    } else {
      res = ServerInfo.settings.health;
    }
...
Meteor.startup(function() {
  connectHandlers
    .use(ServerInfo.settings.healthPath, function(req, res, next) {
      Fiber(function() {
        res.setHeader("content-type", "text/plain");
        return res.end(ServerInfo.health());
      }).run();
    })
    .use(
      ServerInfo.settings.path,
...
function getEc2Metadata() {
  ...
  _.each(lines, function(line) {
    ...
    if (vals.length == 2) ec2[vals[0]] = vals[1];
  });
...
function getExtras() {
  if (ServerInfo.settings.extras) {
    ...
    else return ServerInfo.settings.extras;
  }
}
  ...
  _.each(
    ...
    function(muxer) {
      _.each(muxer._handles, function(handle) {
        ...
        if (driver._usesOplog) logStat("oplogObserveHandles", collectionName);
        else logStat("pollingObserveHandles", collectionName);
      });
    }
  );
